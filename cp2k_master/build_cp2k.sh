...
if [ -z "$CMAKE_OPTS" ]; then
  CMAKE_OPTS="-DCP2K_DATA_DIR=$SCRATCH_REPO/data -DCP2K_USE_DLAF=OFF -DCP2K_USE_PEXSI=OFF -DCP2K_USE_DFTD4=OFF -DCP2K_USE_ELPA=OFF -DCP2K_USE_SIRIUS=OFF -DP2K_USE_MPI=ON"
fi
