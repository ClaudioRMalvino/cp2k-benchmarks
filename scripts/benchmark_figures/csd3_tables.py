#!/usr/bin/env python3
"""Render the tabular .csv files in results/scaling_csd3/ as thesis-ready PDFs (written to plots/scaling_csd3/)."""
import os
...
DATA_DIR = "/home/crm98/cp2k-benchmarks/results/scaling_csd3"   # input CSVs
OUT_DIR = "/home/crm98/cp2k-benchmarks/plots/scaling_csd3"      # rendered table PDFs
...
def table_size_scaling_summary():
    ...
    src = os.path.join(DATA_DIR, "size_scaling_summary_table.csv")
    df = pd.read_csv(src)
    ...
    render_table_pdf(
        df,
        os.path.join(OUT_DIR, "size_scaling_summary_table.pdf"),
...
def table_statistical_comparison():
    ...
    src = os.path.join(DATA_DIR, "statistical_comparison_N1024.csv")
    raw = pd.read_csv(src)
    ...
    render_table_pdf(
        df,
        os.path.join(OUT_DIR, "statistical_comparison_N1024.pdf"),
...
def table_power_law_fits():
    print("\n[3/3] power_law_fits")
    src = os.path.join(DATA_DIR, "power_law_fits.csv")
    raw = pd.read_csv(src)
    df = pd.DataFrame({
        "Branch":             raw["branch"],
    ...
    render_table_pdf(
        df,
        os.path.join(OUT_DIR, "power_law_fits.pdf"),
