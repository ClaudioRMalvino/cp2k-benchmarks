...
mkdir -p /local/data/public/crm98/cp2k_benchmarks/logs/

# Rebuilds and recompiles CP2k with the latest changes to the optimized branch
cd /local/data/public/crm98/cp2k_benchmarks/cp2k_optimized/
./rebuild_cp2k.sh 
...
cd /local/data/public/crm98/cp2k_benchmarks/scripts/
