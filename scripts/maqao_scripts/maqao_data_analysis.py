...
OUTDIR_FIG = "/home/crm98/cp2k-benchmarks/plots/maqao_plots"    # figures (pdf)
...
def write_fresh_branch_aggregate():
    ...
    csv_p = os.path.join(OUTDIR_DATA, "fresh_branch_aggregate.csv")
    txt_p = os.path.join(OUTDIR_DATA, "fresh_branch_aggregate.txt")
    agg.to_csv(csv_p)
...
def plot_fresh_vec_ratio_distribution():
    ...
    p = os.path.join(OUTDIR_FIG, "fresh_vec_ratio_distribution.pdf")
...
def plot_fresh_speedup_with_baseline(top_n=8):
    ...
    ax.grid(axis="y", ls="--", alpha=0.35)
    fig.tight_layout()
    p = os.path.join(OUTDIR_FIG, "fresh_speedup_with_baseline.pdf")
    fig.savefig(p, dpi=300, bbox_inches="tight"); plt.close(fig)
    print(f"  saved {p}")
...
def write_methodology_md():
    ...
    md.append("LProf is the dynamic sampling component of MAQAO ONE View and is "
              ...
              "desired.\n")
    out = os.path.join(OUTDIR_DATA, "maqao_methodology.md")
...
def _ensure_outdir():
    os.makedirs(OUTDIR_FIG, exist_ok=True); os.makedirs(OUTDIR_DATA, exist_ok=True)


def _save_table(df_or_text, basename, header=None):
    """Save a DataFrame or text block as .txt (+ .csv if DataFrame)."""
    txt_path = os.path.join(OUTDIR_DATA, basename + ".txt")
    csv_path = os.path.join(OUTDIR_DATA, basename + ".csv")
...
def plot_hotspot_stackbar():
    ...
    fig.tight_layout()
    path = os.path.join(OUTDIR_FIG, "maqao_hotspot_stackbar.pdf")
...
def plot_array_access_focused():
    ...
    path = os.path.join(OUTDIR_FIG, "array_access_efficiency_focused.pdf")
    fig.savefig(path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"  saved {path}")
...
def write_disclaimer():
    ...
    path = os.path.join(OUTDIR_DATA, "array_access_efficiency_disclaimer.md")
...
def main():
    ...
    for key, _, fn in SECTIONS:
        if args.only and args.only != key:
            continue
        fn()
    print(f"\nDone. Figures in {OUTDIR_FIG}, tables in {OUTDIR_DATA}")
