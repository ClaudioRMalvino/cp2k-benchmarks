# CSD3 RPA transport campaign (2026-07-30 brief)

Full RPA-level transport + thermodynamics series for NaCl(aq) on the
optimised branch (committee models; the 1 m MP2 anchor is re-analysed
through the same pipeline), plus the measured optimised-vs-master cost
proof. Analyzer outputs are level-separated: mp2* labels write to
results/mp2_transport/, rpa* labels to results/rpa_transport/;
cross-level figures and summaries live in results/transport_comparison/.
...
`~/cp2k-benchmarks/results/rpa_transport/timings.csv` via `log_timing.sh`.
...
master's cost). Raw rows in results/rpa_transport/timings.csv.
